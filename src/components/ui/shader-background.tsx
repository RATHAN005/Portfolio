...
const ShaderBackground = ({ className = "" }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    // ResizeObserver watches the actual rendered size of the parent element.
    // This is more reliable than clientWidth at mount time (which can be 0).
    ...
    if (parent && typeof ResizeObserver !== "undefined") {
      ...
      ro.observe(parent);
      // Also set immediately in case it already has size
      const { clientWidth: w, clientHeight: h } = parent;
      setSize(w > 0 ? w : window.innerWidth, h > 0 ? h : window.innerHeight);
    } else {
